...
// Gera um valor aleatório entre inicioi e fim, inclusive
...
void mostra_elementos(int array_int[], int num_eltos) {
    ...
    for (int indice = 0; indice <= num_eltos - 1; ++indice) {
        ...
        // Muda de linha quando a posição do elemento (indice + 1) for múltipl0 de 10
        if ((indice + 1) % 10 == 0)
            printf("\n");
    }
    return;
